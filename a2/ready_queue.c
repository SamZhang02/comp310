...
// ready queue is a singleton linked list with nodes
// that are processes lining up to be executed
QueueNode *head = NULL;
...
void print_ready_queue() {
  ...
  while (cur != NULL) {
    printf("\tPID: %d, length: %d, priority: %d\n", cur->pcb->pid,
           cur->pcb->job_length_score, cur->pcb->priority);
    cur = cur->next;
  }
}
...
bool is_ready_empty() { return head == NULL; }
